    ...
    for user, i_seen in zip(user_ids, seen_item_ids):
        ...
        for item in items:
            if item not in i_seen:
                item_embed = V[item]
                score = user_embed.reshape(1, -1) @ item_embed.reshape(1, -1)
                scores[item] = score
...
class MF(nn.Module):
    ...
    def forward(self, user: torch.Tensor, item: torch.Tensor) -> torch.Tensor:
        ...
        if the training data_2 is small we can deal with it in a single forward pass,
...
def train(model: nn.Module, train_data_inter: np.ndarray, epochs: int, optimizer, loss_func) -> list:
    ...
    train_data_inter - np.ndarray - interaction matrix of the training data_2.
...
def split_interactions(inter_file='sampled_1000_items_inter.txt',
    ...
    '''
    inter_file - string - path to the file with interaction data_2 in LFM2B format;
    ...
    print("Train data_2: ", len(train))
    print("Test data_2: ", len(test))
...
def inter_matr_binary(usr_path, itm_path, inter_path, threshold=1):
    '''
    usr_path - string path to the file with users data_2;
    itm_path - string path to the file with item data_2;
    inter_path - string path to the file with interaction data_2;
    ...
    # reading the three data_2 files
